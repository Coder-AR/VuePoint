//Generated by Claude 4.6 & Gemini (Refactored with "What-If" Grade Simulator sandbox)
    ...
    <body>
      ...
      <script>
        ...
        async function login() {
          ...
          // Inject empty mock array wrapper structure into incoming courses
        ...
        // Helper: Parses standard score strings like "18.00 / 20.00" or fractions into clean numbers
        ...
        // Dual Engine Math Solver: Preserves raw data framework, running point-pool simulations locally
        function calculateBlendedCourseGrade(course) {
          let totalEarned = 0;
          let totalPossible = 0;
          let validPointsFound = false;

          // Compute verified data points
          ...
          // Compute user injected data points
        ...
        function viewCourseAssignments(courseIndex) {
          ...
          // Dynamically detect category groups currently running in this gradebook layout
        ...
        function renderModalAssignmentsList() {
          ...
          // Calculate current sandbox value metrics
          ...
          // Interlace live data records with simulation assets
          let rowsHtml = '';

          // 1. Render Active Mock Items
          ...
          // 2. Render Verified Gradebook Items
          ...
          renderMainGradeTable(); // Keep parent workspace dashboard layout completely aligned in background
        }
...
app.get('/api/search', async (req, res) => {
  ...
  try {
    ...
    if (!districts) return res.status(404).json({ error: "No districts found matching criteria." });
    const districtList = Array.isArray(districts) ? districts : [districts];
    res.json({ districts: districtList.map(d => ({ name: d.Name || 'Unknown', url: d.PvueURL || 'N/A' })) });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// API ENDPOINT: Log In & Compile Base Payload Architecture
