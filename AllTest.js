//Generated by Claude 4.6 & Gemini (Refactored for Student Photo & Dynamic Report Card Matrix)
  ...
  res.send(`
    ...
    <body>
      ...
      <div id="dashboardBox" style="display: none;">
        ...
        <!-- NEW REPORT CARD CONTAINER VIEW -->
        <h3>Finalized Report Card</h3>
        <div class="table-container">
          <table>
            <thead>
              <tr id="reportCardHeadRow">
                <th>Course Title</th>
                <!-- Term Columns are appended here dynamically via Javascript -->
              </tr>
            </thead>
      ...
      <script>
        ...
        async function login() {
          ...
          // NEW: Render Dynamic Report Card Matrix Grid
          if (result.grades && result.grades.length > 0) {
            // 1. Scan and isolate every unique reporting milestone name (Q1, Q2, S1, etc.)
            const uniqueTerms = [];
            ...
            // 2. Append columns dynamically into HTML header row
            uniqueTerms.forEach(term => {
              reportCardHeadRow.innerHTML += '<th style="text-align: center;">' + term + '</th>';
            });

            // 3. Populate matching grades cell data rows
            reportCardTableBody.innerHTML = result.grades.map(course => {
              ...
              uniqueTerms.forEach(term => {
                ...
                if (match && match.grade && match.grade !== 'N/A') {
                  ...
                  rowHtml += '<td style="text-align: center; color:#cbd5e1; font-weight:bold;">-</td>'; // grading period not finalized/opened yet
                }
              });
...
app.post('/api/login', async (req, res) => {
  ...
  try {
   ...
   // Grades & GPA (UPDATED: Maps complete historical array matrices)
    ...
    if (courses) {
      ...
      grades = courseList.map(course => {
        ...
        // Isolate complete marks array block to build historical timeline array
        ...
        return {
          ...
          history: historyData // attached list arrays passed to UI matrix
        };
      });
    }
